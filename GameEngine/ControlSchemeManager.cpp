...
ControlSchemeManager::ControlSchemeManager()
{
	...
	action_control_state[Action::MOVE_LEFT] = ControlState::RELEASED;
	action_control_state[Action::MOVE_RIGHT] = ControlState::RELEASED;
	action_control_state[Action::JUMP] = ControlState::RELEASED;
}
...
void ControlSchemeManager::Serialize()
{
	...
	for (auto element : control_map) {
		 ...
		 */
		action_enum_index = static_cast<Action>(element.second["Enum Key"].get<int>());
		 ...
		 */
		control_code_struct_vec = element.second["Control Codes"].get<std::vector<std::vector<int>>>();
